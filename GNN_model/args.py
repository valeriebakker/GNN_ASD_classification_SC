# class Args:
#     def __init__(self):
#         self.dropout = 0.130		     # Fraction of node features randomly set to 0 during training to reduce overfitting.
#         self.edge_dropout = 0.125	     # Fraction of edges randomly dropped during training to regularize the graph structure.
#         self.epochs = 500                # Maximum number of training epochs.
#         self.weight_decay = 0.0072       # L2 regularization on the weights inside the optimizer. Helps prevent very large weights.
#         self.lr = 	0.0001               # Learning rate (step size) for the optimizer.
...
#         self.use_pheno_data = False                  # Whether to include phenotypic similarity in the edge weights.
#         self.graph_method = "aff_threshold"         # Graph construction method: threshold edges or use kNN.

#         if self.graph_method == "aff_threshold":
#             self.affinity_threshold = 0.656         # Similarity threshold below which edges are pruned.
#             self.k = None
